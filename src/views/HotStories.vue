...
export default { 
    ...
    async created() {
        // koga kje se kreira objektot kje gi fetchne od api best stories id
        const bestStoriesId = await fetch(
            "https://hacker-news.firebaseio.com/v0/beststories.json"
        ).then((res) => res.json());
        // rezultatot go pravi vo json i go stava vo bestStoriesId
        
        //await zavrshuva celosno da se zavrshi eden promise pred da prodolzhi so kodot,
        // istiot mozhe da se fullfilne ili da se rejectne (pri shto mozhe da se koristi error collection)
        this.stories = await Promise.all( //promise all bidejki 50 promises kje se izvrsuvaat
            bestStoriesId.slice(0, 50).map((id) => 
